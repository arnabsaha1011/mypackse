...
#Long Open Issues (>20 days)
SELECT ISSUE_ID, USER_GROUP_NUMBER, USER_NAME FROM ISSUE JOIN USER ON USER_ID = ISSUE_USER_ID WHERE DATEDIFF(ISSUE_CLOSE_TIME, ISSUE_CREATE_TIME) > 20 order by USER_GROUP_NUMBER;

#Short Open Issues (<1 hr)
SELECT ISSUE_ID, TIMESTAMPDIFF(MINUTE,ISSUE_CREATE_TIME, ISSUE_CLOSE_TIME ), USER_GROUP_NUMBER, USER_NAME from ISSUE join USER on ISSUE_USER_ID = USER_ID where TIMESTAMPDIFF(HOUR,ISSUE_CREATE_TIME, ISSUE_CLOSE_TIME )<1 order by USER_GROUP_NUMBER;

# Issues Missing Milestones
select ISSUE_ID, ISSUE_USER_NAME, USER_GROUP_NUMBER from ISSUE join 
USER on USER_ID = ISSUE_USER_ID where ISSUE_MILESTONE_ID = "" order by USER
_GROUP_NUMBER;
